...
class DGMR():
  ...
  def __init__(self, generator_obj = None,
                discriminator_obj = None,
               epochs = 3, strategy = None, batch_size = 12,
    ...
    self._gen_op = tf.keras.optimizers.Adam(5e-5, beta_1=0.0), # adapted
    self._disc_op = tf.keras.optimizers.Adam(2e-4, beta_1=0.0), # adapted
    self._epochs = epochs
    ...
    ####      ####
      ...
      for epoch in range(self._epochs):
        ...
        for step in tf.range(self.steps_per_epoch):
          ...
          #   tf.summary.scalar('Discriminator Loss', data=summed_disc_loss, step=tf.cast(step, tf.int64))
          #   tf.summary.scalar('Generator Loss', data=summed_gen_loss, step=tf.cast(step, tf.int64))
          ...
          if step % self.eval_step == 0:
            ...
            for valid_step in tf.range(self.num_valid):
              ...
              if not data.has_value():
                break
              loss_ = self.distributed_valid_step(valid_data.get_value())
              if first_loss:
                loss = loss_
                first_loss = False
              else: loss = loss + loss_
            loss = loss / self.num_valid
            valid_disc_loss, valid_gen_loss = tf.split(loss, 2, axis=0)
            with self.writer.as_default():
              tf.summary.scalar('Validation Disc Loss', data=valid_disc_loss, step=tf.cast(step, tf.int64))
              tf.summary.scalar('Validation Gen Loss', data=valid_gen_loss, step=tf.cast(step, tf.int64))
  ...
  def train_step_2(self, frames):
    ...
    for _ in range(1):
      ...
      real_sequence = tf.concat([batch_inputs, batch_targets], axis=1)
      concat_inputs = tf.concat([real_sequence, gen_sequence], axis=0)
      tf.print(concat_inputs)
      with tf.GradientTape() as disc_tape:
        tf.print(self._discriminator)
        concat_outputs = self._discriminator(concat_inputs)
        score_real, score_generated = tf.split(concat_outputs, 2, axis=0)
    ...
    for _ in range(1):
      ...
      return tf.concat([disc_loss_dist, gen_loss], axis=0)
